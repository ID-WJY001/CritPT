...
CODE=${ROOT}/code
SESSION=${SESSION:-rl-14b-smoke}
CONFIG=${CONFIG:-configs/experiments/qwen3_14b_grpo_verl_smoke.env}
LOG=${ROOT}/logs/qwen3_14b_grpo_critpt_a10040g_smoke.log
...
if tmux has-session -t "${SESSION}" 2>/dev/null; then
  ...
  echo "tail:   bash scripts/ops/tail_14b_log.sh"
  exit 0
fi
