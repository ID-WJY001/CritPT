...
TARGET_MODEL="${TARGET_MODEL:-${RL_DATA_ROOT}/models/qwen3-8b-e3-yunwu-final-answer-judge-from-e2-base-gs${STEP}}"
CONFIG_MODEL="${CONFIG_MODEL:-${RL_DATA_ROOT}/models/qwen3-8b-e2-yunwu-semantic-judge-base-gs120}"
...
run_eval() {
  local style="$1"
  local suffix="$2"
  local label="qwen3_8b_e3_yunwu_final_answer_judge_from_e2_base_gs${STEP}"
  local out_dir="${RL_DATA_ROOT}/logs/eval/${label}_official70_${suffix}_ctx32768_max4096"
  echo "[e3 official70] generating ${style} -> ${out_dir}"
