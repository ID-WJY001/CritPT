...
def summarize(examples: list[SyntheticCritPTExample]) -> dict:
    return {
        ...
        "focus": dict(Counter(summarize_v20_source(example) for example in examples)),
...
def verify_targets(examples: list[SyntheticCritPTExample], workers: int) -> None:
    ...
    if workers <= 1:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            for example, (ok, reason) in zip(
                examples,
                executor.map(verify_v20_example, examples, chunksize=16),
                strict=True,
            ):
                if not ok:
                    failures.append((example.problem_id, reason))
    if failures:
        ...
        raise SystemExit(f"{len(failures)} V20 examples failed verification:\n{preview}")
...
def write_sample_markdown(path: Path, examples: list[SyntheticCritPTExample], per_split: int) -> None:
    ...
    lines = [
        "# V20 Focused-Hard 数据抽样",
        "",
        "V20 根据 V19 前 12 step 肉眼问题设计，重点覆盖 operator canonical label、empty interval filter、HHG/OAM runtime-safe 三类。",
        "它仍然不使用官方测试题原文，只使用 synthetic verifier 题。",
        "",
    ]
    for example in selected:
        lines.extend(
            [
                ...
                f"- focus: `{summarize_v20_source(example)}`",
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("artifacts/data/v20_focused_hard"))
    parser.add_argument("--name", default="synthetic_critpt_v20_focused_hard")
    ...
    train = generate_v20_focused_hard_examples(args.train_size, args.seed, "train")
    val = generate_v20_focused_hard_examples(args.val_size, args.seed + 1, "val")
    test = generate_v20_focused_hard_examples(args.test_size, args.seed + 2, "test")
    ...
    manifest = {
        ...
        "leakage_policy": {
            ...
            "note": "V20 is synthetic and focuses on failure modes found in V19 rollouts; no official prompt text is copied.",
        },
        "quality_gates": {
            ...
            "focused_operator_canonicalization": True,
            "focused_empty_interval_filters": True,
            "focused_hhg_runtime_safety": True,
