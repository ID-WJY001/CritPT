...
def summarize(examples: list[SyntheticCritPTExample]) -> dict:
    return {
        "total": len(examples),
        "source": dict(Counter(summarize_v14_source(example) for example in examples)),
    ...
    if workers <= 1:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            for example, (ok, reason) in zip(
                examples, executor.map(verify_v14_example, examples, chunksize=16), strict=True
            ):
                if not ok:
                    failures.append((example.problem_id, reason))
    if failures:
        preview = "\n".join(f"{problem_id}: {reason}" for problem_id, reason in failures[:25])
        raise SystemExit(f"{len(failures)} V14 examples failed verification:\n{preview}")
...
def write_sample_markdown(path: Path, examples: list[SyntheticCritPTExample], per_split: int) -> None:
    ...
    lines = [
        "# V14 Compact-Exec 数据抽样",
        "",
        "V14 在 V13 official-style 的基础上，额外加入官方评测暴露出的 failure case：",
        "长稀疏系数、BNS/算符集合、闭合符号表达式、紧凑序列生成。",
        "目标不是用官方原题训练，而是训练模型不要 runaway，且输出可执行的 `answer()`。",
        "",
    ]
    for example in selected:
        lines.extend(
            [
                ...
                f"- source: `{summarize_v14_source(example)}`",
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("artifacts/data/v14_compact_exec"))
    parser.add_argument("--name", default="synthetic_critpt_v14_compact_exec")
    ...
    train = generate_v14_compact_exec_examples(args.train_size, args.seed, "train")
    val = generate_v14_compact_exec_examples(args.val_size, args.seed + 1, "val")
    test = generate_v14_compact_exec_examples(args.test_size, args.seed + 2, "test")
    ...
    manifest = {
        ...
        "leakage_policy": {
            ...
            "note": "V14 uses synthetic hardcases inspired by observed failure modes, not official prompt text.",
        },
        "quality_gates": {
