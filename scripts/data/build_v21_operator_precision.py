...
from rl_posttrain.critpt_synth.v21_operator_precision import (
    generate_v21_operator_precision_examples,
    summarize_v21_source,
    verify_v21_example,
)
...
def summarize(examples: list[SyntheticCritPTExample]) -> dict:
    return {
        ...
        "focus": dict(Counter(summarize_v21_source(example) for example in examples)),
...
def verify_targets(examples: list[SyntheticCritPTExample], workers: int) -> None:
    ...
    if workers <= 1:
        for example in examples:
            ok, reason = verify_v21_example(example)
            if not ok:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            for example, (ok, reason) in zip(
                examples,
                executor.map(verify_v21_example, examples, chunksize=16),
                strict=True,
            ):
                if not ok:
                    failures.append((example.problem_id, reason))
    if failures:
        ...
        raise SystemExit(f"{len(failures)} V21 examples failed verification:\n{preview}")
...
def write_sample_markdown(path: Path, examples: list[SyntheticCritPTExample], per_split: int) -> None:
    ...
    lines = [
        "# V21 Operator-Precision 数据抽样",
        "",
        "V21 继续使用 synthetic verifier 题，不复制官方题原文。重点是 operator canonical label 的精确筛选：",
        "不要返回候选全集，不要写非 canonical power，不要靠长列表碰运气。",
        "",
    ]
    for example in selected:
        lines.extend(
            [
                ...
                f"- focus: `{summarize_v21_source(example)}`",
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("artifacts/data/v21_operator_precision"))
    parser.add_argument("--name", default="synthetic_critpt_v21_operator_precision")
    ...
    train = generate_v21_operator_precision_examples(args.train_size, args.seed, "train")
    val = generate_v21_operator_precision_examples(args.val_size, args.seed + 1, "val")
    test = generate_v21_operator_precision_examples(args.test_size, args.seed + 2, "test")
    ...
    manifest = {
        "name": args.name,
        "profile": "v21_operator_precision",
        "seed": args.seed,
        ...
        "leakage_policy": {
            ...
            "note": "V21 is synthetic and targets operator over-enumeration observed in V20 rollouts; no official prompt text is copied.",
        },
        "quality_gates": {
            ...
            "anti_overenumeration_prompt": True,
