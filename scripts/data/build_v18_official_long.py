...
from rl_posttrain.critpt_synth.v18_official_long import (
    generate_v18_official_long_examples,
...
def summarize(examples: list[SyntheticCritPTExample]) -> dict:
    return {
        "total": len(examples),
        "source": dict(Counter(summarize_v18_source(example) for example in examples)),
        ...
        "prompt_style": dict(
            Counter(str(example.metadata.get("prompt_style", "unknown")) for example in examples)
        ),
    }
    ...
    if workers <= 1:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            for example, (ok, reason) in zip(
                examples, executor.map(verify_v18_example, examples, chunksize=16), strict=True
            ):
                if not ok:
                    failures.append((example.problem_id, reason))
    if failures:
        preview = "\n".join(f"{problem_id}: {reason}" for problem_id, reason in failures[:25])
        raise SystemExit(f"{len(failures)} V18 examples failed verification:\n{preview}")
...
def write_sample_markdown(path: Path, examples: list[SyntheticCritPTExample], per_split: int) -> None:
    ...
    lines = [
        "# V18 Official-Long Hard-Mix 数据抽样",
        "",
        "V18 不使用官方测试题原文。它把 V15/V17 的可验证 hard cases 包装成更长的 official-like notebook prompt。",
        "目标是训练模型在长上下文中定位真正的 `Parsing template`，不要返回占位 0，不要写散文，不要 runaway。",
        "",
    ]
    for example in selected:
        lines.extend(
            [
                ...
                f"- source: `{summarize_v18_source(example)}`",
                ...
                f"- wrapper_hash: `{example.metadata.get('v18_wrapper_hash')}`",
                "",
                "### Prompt",
                "",
                "```text",
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("artifacts/data/v18_official_long"))
    parser.add_argument("--name", default="synthetic_critpt_v18_official_long")
    ...
    train = generate_v18_official_long_examples(args.train_size, args.seed, "train")
    val = generate_v18_official_long_examples(args.val_size, args.seed + 1, "val")
    test = generate_v18_official_long_examples(args.test_size, args.seed + 2, "test")
    ...
    manifest = {
        "name": args.name,
        "profile": "v18_official_long_hardmix",
        "seed": args.seed,
        ...
        "leakage_policy": {
            ...
            "note": "V18 wraps synthetic hard cases in official-like long context; it does not copy official test prompt text.",
        },
        "quality_gates": {
